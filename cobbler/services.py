...
class CobblerSvc(object):
    ...
    def autoinst(self, profile=None, system=None, REMOTE_ADDR=None, REMOTE_MAC=None, **rest):
        """
        Generate automatic installation files
        """
        self.__xmlrpc_setup()
        data = self.remote.generate_autoinst(profile, system, REMOTE_ADDR, REMOTE_MAC)
        return u"%s" % data
    ...
    def find_autoinst(self, system=None, profile=None, **rest):
        ...
        if system is not None:
            url = "%s/cblr/svc/op/autoinst/system/%s" % (serverseg, name)
        elif profile is not None:
            url = "%s/cblr/svc/op/autoinst/profile/%s" % (serverseg, name)
        else:
