...
class AutoInstallationGen:
    ...
    def generate_autoinstall_for_system(self, sys_name):
        ...
        return self.generate_autoinst(profile=p, system=s)

    def generate_autoinst(self, profile=None, system=None):
        ...
        autoinstall_path = meta["autoinst"]

        if not autoinstall_path:
            return "# automatic installation file is missing or invalid: %s" % meta["autoinst"]
        ...
        try:
            ...
            if raw_data is None:
                return "# automatic installation file is sourced externally: %s" % meta["autoinst"]
            ...
            self.api.logger.warning("automatic installation file not found: %s" % meta["autoinst"])
            return "# automatic installation file not found: %s" % meta["autoinst"]
    ...
    def generate_autoinstall_for_profile(self, g):
        ...
        return self.generate_autoinst(profile=g)
