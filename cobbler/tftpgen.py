...
class TFTPGen:
    ...
    def copy_single_distro_files(self, d, dirtree, symlink_ok):
        ...
        if "nexenta" == d.breed:
            ...
            shutil.copytree(os.path.join('/var', 'lib', 'cobbler', 'autoinsts', 'install_profiles'),
    ...
    def write_pxe_file(self, filename, system, profile, distro, arch,
        ...
        if image is None:
            ...
            # Find the autoinst if we inherit from another profile
            if system:
                blended = utils.blender(self.api, True, system)
            else:
                blended = utils.blender(self.api, True, profile)
            autoinstall_path = blended.get("autoinst", "")
            ...
            if format == "grub":
                ...
                if system.netboot_enabled:
                    ...
                    if arch.startswith("ppc"):
                        ...
                        # a standard autoinst and different options - so giving it a dedicated
    ...
    def build_kernel_options(self, system, profile, distro, image, arch,
        ...
        # autoinst path rewriting (get URLs for local files)
        if autoinstall_path is not None and autoinstall_path != "":
            ...
            if system is not None and autoinstall_path.startswith("/"):
                autoinstall_path = "http://%s/cblr/svc/op/ks/system/%s" % (ipaddress, system.name)
            elif autoinstall_path.startswith("/"):
                autoinstall_path = "http://%s/cblr/svc/op/ks/profile/%s" % (ipaddress, profile.name)
    ...
    def generate_gpxe(self, what, name):
        ...
        if what == "profile":
            blended['append_line'] = self.build_kernel_options(None, obj, distro, None, None, blended['autoinst'])
        else:
            blended['append_line'] = self.build_kernel_options(obj, None, distro, None, None, blended['autoinst'])
