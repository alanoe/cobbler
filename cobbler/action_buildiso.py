    ...
    def generate_netboot_iso(self, imagesdir, isolinuxdir, profiles=None, systems=None, exclude_dns=None):
        ...
        for profile in all_profiles:
            if profile.name in which_profiles or do_all_profiles is True:
                ...
                if data["autoinst"].startswith("/"):
                    data["autoinst"] = "http://%s:%s/cblr/svc/op/ks/profile/%s" % (
                ...
                if dist.breed == "suse":
                    ...
                    if "autoyast" in data["kernel_options"] and data["kernel_options"]["autoyast"] != "":
                        ...
                        append_line += " autoyast=%s" % data["autoinst"]

                if dist.breed == "redhat":
                    if "proxy" in data and data["proxy"] != "":
                        append_line += " proxy=%s http_proxy=%s" % (data["proxy"], data["proxy"])
                    append_line += " ks=%s" % data["autoinst"]

                if dist.breed in ["ubuntu", "debian"]:
                    append_line += " auto-install/enable=true url=%s" % data["autoinst"]
                    if "proxy" in data and data["proxy"] != "":
        ...
        for system in all_systems:
            ...
            if system.name in which_systems or do_all_systems is True:
                ...
                if data["autoinst"].startswith("/"):
                    data["autoinst"] = "http://%s:%s/cblr/svc/op/ks/system/%s" % (
                ...
                if dist.breed in ["ubuntu", "debian"]:
                    append_line += " auto-install/enable=true url=%s netcfg/disable_dhcp=true" % data["autoinst"]
                    if "proxy" in data and data["proxy"] != "":
                        append_line += " mirror/http/proxy=%s" % data["proxy"]
    ...
    def generate_standalone_iso(self, imagesdir, isolinuxdir, distname, filesource):
        ...
        for descendant in descendants:
            ...
            if descendant.COLLECTION_TYPE == 'profile':
                autoinstall_data = self.api.kickgen.generate_autoinstall_for_profile(descendant.name)
            elif descendant.COLLECTION_TYPE == 'system':
                autoinstall_data = self.api.kickgen.generate_autoinstall_for_system(descendant.name)
            ...
            ks_file.write(autoinstall_data)
            ks_file.close()
